...
if method == 1:
    plt.scatter(ec/10**15, M, s=3,label=str(name))
    plt.xlabel(r'$\varepsilon_c$ [g cm$^{-3}$]$\times10^{15}$', fontsize='15')
    ...
    plt.ylabel(r'$M$ [M$_\odot$]', fontsize='20')
    ...
    plt.xlabel(r'$\nu$ [Hz]', fontsize='20')
    plt.ylabel(r'$(M-M_*)/M_*$', fontsize='20')
    plt.legend(fontsize='15')
    plt.xticks(fontsize = 12)
    plt.yticks(fontsize = 12)
    ...
    plt.scatter(delta**2, (M-Mstat)/Mstat, s=3, label=str(name))
    plt.xlabel(r'$\Omega^2 (R_*^3 / GM_*)$', fontsize='15')
